...
class TestSubMenu:
    def test_create_submenu(self, client, menu, submenu, db):
        ...
        resp = client.post(f"/api/v1/menus/{menu_id}/submenus/", data=json.dumps(submenu))
        resp_data = resp.json()
    ...
    def test_update_submenu(self, client, menu, submenu, create_menu, db):
        ...
        last_submenu = db.query(SubMenu).first()
        submenu_id = str(last_submenu.id)
        resp = client.patch(f"/api/v1/menus/{menu_id}/submenus/{submenu_id}/", data=json.dumps(new_data))
        resp_data = resp.json()
        assert resp_data["id"] == submenu_id
